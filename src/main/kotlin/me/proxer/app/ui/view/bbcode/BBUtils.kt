...
internal object BBUtils {
    internal fun cutAttribute(target: String, regex: Regex): String? = regex
        .find(target)
        ?.groupValues
        ?.getOrNull(1)
        ?.trim()
...
internal inline fun <reified T : View> applyToViews(views: List<View>, noinline operation: (T) -> Unit) = views.apply {
    asSequence()
        .flatMap { sequenceOf(it) + ((it as? ViewGroup)?.recursiveChildren ?: emptySequence()) }
