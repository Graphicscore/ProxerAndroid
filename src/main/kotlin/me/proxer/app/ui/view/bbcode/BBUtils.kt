...
internal object BBUtils {
    ...
    internal fun findBaseActivity(currentContext: Context): BaseActivity? =
        when (currentContext) {
...
internal inline fun applyToAllViews(
    views: List<View>,
    noinline operation: (View) -> Unit,
) = views.apply {
    asSequence()
...
internal inline fun CharSequence.toSpannableStringBuilder() =
    this as? SpannableStringBuilder
        ?: SpannableStringBuilder(this)

internal fun SpannableStringBuilder.trimStartSafely() =
    when (firstOrNull()?.isWhitespace()) {
        true -> {
            indices
...
internal fun SpannableStringBuilder.trimEndSafely() =
    when (lastOrNull()?.isWhitespace()) {
        true -> {
            indices
                .reversed()
                .firstOrNull { !this[it].isWhitespace() }
