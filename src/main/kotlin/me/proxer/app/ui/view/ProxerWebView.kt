...
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private fun replaceIFrames(context: Context, html: String): String = html.replace(iFrameRegex) { matchResult ->
        val url = matchResult.groupValues[1]

        """<a href="$url">${context.getString(R.string.view_web_iframe_link)}</a>"""
    }

    private fun upgradeImageLinks(html: String): String = html.replace(imageHttpRegex) { matchResult ->
    ...
    private class ProxerWebClient : WebViewClient() {
        ...
        override fun shouldOverrideUrlLoading(view: WebView?, request: WebResourceRequest): Boolean =
            when (val httpUrl = request.url.toString().toPrefixedUrlOrNull()) {
                null -> {
        ...
        private fun loadImage(view: WebView, url: HttpUrl, fileExtension: String): WebResourceResponse? {
            return try {
                val imageFile =
                    Glide
                        .with(view)
