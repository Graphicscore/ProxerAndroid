...
class MessengerWorker(
    ...
    private fun synchronize(): Map<LocalConference, List<LocalMessage>> {
        ...
        val newConferencesAndMessages =
            try {
                markConferencesAsRead(
                    messengerDao
                        .getConferencesToMarkAsRead()
                        .asSequence()
                ...
                fetchConferences().associate { conference ->
                    fetchNewMessages(conference).let { (messages, isFullyLoaded) ->
                        val isLocallyFullyLoaded =
                            messengerDao
                                .findConference(conference.id.toLong())
                                ?.isFullyLoaded == true
    ...
    private fun fetchForExistingConference(
        ...
        var existingUnreadMessageAmount =
            messengerDao.getUnreadMessageAmountForConference(
    ...
    private fun showNotification(context: Context) {
        val unreadMap =
            messengerDao.getUnreadConferences().associateWith {
                messengerDao
