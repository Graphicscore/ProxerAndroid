...
object ProxerLibExtensions {
    fun fskConstraintFromAppString(
        context: Context,
        string: String,
    ) = when (string) {
...
fun MediaLanguage.toGeneralLanguage() =
    when (this) {
...
fun MediaLanguage.toAnimeLanguage() =
    when (this) {
...
fun AnimeLanguage.toMediaLanguage() =
    when (this) {
...
fun Language.toAppDrawable(context: Context) =
...
fun Country.toAppDrawable(context: Context) =
    AppCompatResources.getDrawable(
        context,
        when (this) {
...
fun Category.toEpisodeAppString(
    context: Context,
    number: Int? = null,
): String =
    when (number) {
...
fun MediaState.toAppDrawable(context: Context): IconicsDrawable =
    IconicsDrawable(context).apply {
        icon =
            when (this@toAppDrawable) {
...
fun UserMediaProgress.toEpisodeAppString(
    ...
    when (this) {
        ...
        UserMediaProgress.CANCELLED -> {
            context.getString(R.string.user_media_progress_cancelled, episode)
        }
    }
...
fun FskConstraint.toAppString(context: Context): String =
    context.getString(
        when (this) {
...
fun FskConstraint.toAppDrawable(context: Context) =
    AppCompatResources.getDrawable(
        context,
        when (this) {
...
fun IndustryType.toAppString(context: Context): String =
    context.getString(
        when (this) {
...
fun CalendarDay.toAppString(context: Context): String =
    context.getString(
        when (this) {
...
fun Gender.toAppString(context: Context): String =
    context.getString(
        when (this) {
...
fun RelationshipStatus.toAppString(context: Context): String =
    context.getString(
        when (this) {
...
fun MessageAction.toAppString(
    context: Context,
    username: String,
    message: String,
): String =
    when (this) {
        MessageAction.ADD_USER -> {
            context.getString(R.string.action_conference_add_user, "@$username", "@$message")
        }

        MessageAction.REMOVE_USER -> {
            context.getString(
                R.string.action_conference_delete_user,
                "@$username",
                "@$message",
            )
        }

        MessageAction.SET_LEADER -> {
            context.getString(R.string.action_conference_set_leader, "@$username", "@$message")
        }

        MessageAction.SET_TOPIC -> {
            context.getString(R.string.action_conference_set_topic, "@$username", message)
        }

        MessageAction.EXIT -> {
            context.getString(R.string.action_conference_exit, "@$message")
        }
...
fun MediaType.isAgeRestricted(): Boolean =
    this == MediaType.ALL_WITH_HENTAI || this == MediaType.HENTAI || this == MediaType.HMANGA
...
inline val Entry.isTrulyAgeRestricted: Boolean
    get() =
        isAgeRestricted || medium == Medium.HMANGA || medium == Medium.HENTAI ||
...
fun Conference.toLocalConference(isFullyLoaded: Boolean) =
    LocalConference(
        id.toLong(),
        topic,
        customTopic,
...
fun UcpSettings.toLocalSettings() =
...
fun UserComment.toParsedUserComment() =
    ParsedUserComment(
        id,
        entryId,
        entryName,
...
fun UserMediaListEntry.toLocalEntry() =
...
fun UserMediaListEntry.toLocalEntryUcp() =
    LocalUserMediaListEntry.Ucp(
        id,
        name,
        episodeAmount,
        medium,
...
fun UcpHistoryEntry.toLocalEntryUcp() =
    LocalUserHistoryEntry.Ucp(
        id,
        entryId,
        name,
        language,
...
fun HttpUrl.proxyIfRequired() =
    when (this.hasProxerHost) {
        true -> this
