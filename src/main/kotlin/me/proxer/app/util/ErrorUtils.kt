...
object ErrorUtils {
    ...
    internal fun getMessage(error: Throwable, isLoggedIn: Boolean): Int = when (val innermostError = getInnermostError(error)) {
        is ProxerException -> {
    ...
    private fun getMessageForProxerException(error: ProxerException, isLoggedIn: Boolean) = when (error.errorType) {
        SERVER -> {
