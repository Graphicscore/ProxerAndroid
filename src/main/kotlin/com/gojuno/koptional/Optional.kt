...
fun <A : Any, B : Any> Optional<A>.map(body: (A) -> B): Optional<B> =
    when (this) {
        is Some -> Some(body(value))
        None -> None
    }

fun <A : Any, B : Any> Optional<A>.flatMap(body: (A) -> Optional<B>): Optional<B> =
    when (this) {
        is Some -> body(value)
        None -> None
    }

fun <A : Any> Optional<A>.filter(predicate: (A) -> Boolean): Optional<A> =
    when (this) {
