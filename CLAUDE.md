...
- JVM unit tests exercising `org.threeten.bp.ZoneId.systemDefault()` (e.g. `Instant.toLocalDate()`) throw `ZoneRulesException: No time-zone data files registered`. `threetenabp` ships the "no-tzdb" `threetenbp` classifier, which expects `AndroidThreeTen.init()` to register zone data from Android assets at app startup — that never runs in JVM unit tests. Fix: add `testImplementation "org.threeten:threetenbp:1.7.1"` (the plain, non-classifier jar bundles `tzdb.dat`; the zone provider finds it on the classpath regardless of which jar a given class loaded from). Added in `gradle/dependencies.gradle` for Task 29 (`MediaListViewModelTest`).
