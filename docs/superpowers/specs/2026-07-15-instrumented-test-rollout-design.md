...
**`ServerStatusScreen` is in Group 1 despite being trivial.** Exactly two ViewModels in the app override `isLoginRequired = false` — `ServerStatusViewModel.kt:25` and `ScheduleViewModel.kt:13` — so these are the only two screens that can prove the harness works *without* a login fixture. `ServerStatusScreen` additionally exercises a path no other Group 1 screen touches: it scrapes HTML through the raw Koin-injected `OkHttpClient` (via `CallStringBodySingle`) rather than through `ProxerApi`. Worth one cheap test each.

Note that `MediaListViewModel.kt:52-53` (`super.isLoginRequired || type.isAgeRestricted()`) is **unconditionally true**, since `super.isLoginRequired` is `true` — and `isAgeConfirmationRequired` is defined as `isLoginRequired`, so age confirmation is unconditional too. It is not a conditional login gate despite reading like one.
