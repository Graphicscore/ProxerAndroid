...
- **Industry/TranslatorGroup Info tabs carry a latent first-frame NPE, not fixed here.** Discovered during implementation: both render `data!!` inside `ContentScreen`'s content lambda, which can be composed with `data == null` before `load()` runs — the same bug that crashed `ConferenceInfoScreen` (fixed in this group by changing its loading condition `isLoading == true` → `isLoading != false`). Industry/TranslatorGroup happen to win a frame-timing race via their `HorizontalPager`, so their tests pass and they were left untouched to keep Group 3 from modifying passing screens. Their `isLoading` is seeded `false` (via `observeAsState(false)`), so the one-token condition fix does not protect them — a real fix needs `observeAsState()` + `isLoading != false`, or a `data?.let { … }` guard. Recorded in the instrumented-test-followups memory.
