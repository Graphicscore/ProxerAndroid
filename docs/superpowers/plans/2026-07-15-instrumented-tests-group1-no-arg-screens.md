...
- `src/test/kotlin/me/proxer/app/base/ProxerEndpointTestUtils.kt` (`stubPagingSuccess` etc.) lives in the unit-test source set, so it is **not on the androidTest classpath**. Hand-roll the `ProxerCall` mocks following `NotificationScreenTest.kt:61-77`. Same for `RxTrampolineRule`. `koin-test` (`inject()`, `KoinTestRule`) is likewise unavailable — not because it is JVM-only, but because it is declared `testImplementation` in `gradle/dependencies.gradle`; resolve singletons with `safeInject()` instead.
